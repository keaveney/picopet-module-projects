...
def main():
    ...
    for idx, name in enumerate(OUTPUT_NAMES):
        ...
        pred_df[f"{name}_pred"] = preds[:, idx]
    ...
    pd.DataFrame(history).to_csv(args.outdir / "training-history.csv", index=False)

    training_summary = {
        "best_val_loss": float(best_val),
        "best_val_mean_3d_error_mm": float(min(history["val_spatial_distance"])),
        "bounds": {
            name: [float(bounds_np[i, 0]), float(bounds_np[i, 1])]
            for i, name in enumerate(OUTPUT_NAMES)
        },
        "oob_relative_weights": {
            name: float(oob_relative_weights_np[i])
            for i, name in enumerate(OUTPUT_NAMES)
        },
        "total_pe_window": [args.total_pe_min, args.total_pe_max],
        "sigma_bounds_mm": [float(args.sigma_min_mm), float(args.sigma_max_mm)],
        "z_gmm_components": int(args.z_gmm_components),
        "prediction_csv": "test-predictions-with-uncertainty.csv",
        "training_history_csv": "training-history.csv",
    }

    with open(args.outdir / "training-summary.json", "w") as f:
        json.dump(training_summary, f, indent=2)

    print(f"Wrote {args.outdir / 'test-predictions-with-uncertainty.csv'}")
    print(f"Wrote {args.outdir / 'training-summary.json'}")
