...
# Keep the glass in direct contact with the pixel front face
air_gap = -0.0 * u.mm
...
for ix in range(n):
    for iy in range(n):
        name = f"glass_{ix}_{iy}"
        v = sim.add_volume("Box", name)
        v.mother = "front_glass"
        v.size = [pix_size[0], pix_size[1], glass_thickness]
        v.material = glass_material
        v.translation = [ix * pitch - offset, iy * pitch - offset, 0]
        glass_pixel_names.append(name)
...
front_reflector.color = [1.0, 0.0, 0.0, 1.0]
...
sipm_block.translation = [0, 0, (pixel_back_z + air_gap) + sipm_thickness / 2.0]
...
for ix in range(n):
    for iy in range(n):
        ...
        #v.translation = [ix * pitch - offset, iy * pitch - offset, 0]
        v.translation = [ix * pitch - offset, iy * pitch - offset, (pixel_back_z + air_gap) + sipm_thickness / 2.0]
        v.color = [0.0, 0.0, 1.0, 1.0]
...
#print("glass pixel names = " + str(glass_pixel_names))
#print("sipm pixel names = " + str(sipm_pixel_names))
...
for name in range(0, len(pixel_names)):
    #sim.physics_manager.add_optical_surface(sipm_pixel_names[name],pixel_names[name] , diffuse_surface_name)
    #sim.physics_manager.add_optical_surface(pixel_names[name], sipm_pixel_names[name], diffuse_surface_name)
    sim.physics_manager.add_optical_surface("sipm_block", pixel_names[name] , diffuse_surface_name)
    sim.physics_manager.add_optical_surface(pixel_names[name], "sipm_block", diffuse_surface_name)

# Front diffuser + reflector surfaces (pixels directly to glass)
for name in range(0, len(pixel_names)):
    sim.physics_manager.add_optical_surface(pixel_names[name], "front_glass", diffuse_surface_name)
    sim.physics_manager.add_optical_surface("front_glass", pixel_names[name], diffuse_surface_name)

sim.physics_manager.add_optical_surface("front_glass", "world", esr_surface_name)
sim.physics_manager.add_optical_surface("world", "front_glass", esr_surface_name)
...
#sim.physics_manager.add_optical_surface("sipm_block", "world", esr_surface_name)
#sim.physics_manager.add_optical_surface("world", "sipm_block", esr_surface_name)

for name in sipm_pixel_names:
    sim.physics_manager.add_optical_surface(name, "world", esr_surface_name)
    sim.physics_manager.add_optical_surface("world", name, esr_surface_name)
...
sipm_photons.attached_to = sipm_pixel_names
