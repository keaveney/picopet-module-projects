...
# Simulation world
sim.world.size = [60 * u.cm, 60 * u.cm, 60 * u.cm]
sim.world.material = "G4_AIR"

# Physics 
sim.physics_manager.physics_list_name = "G4EmStandardPhysics_option4"
sim.physics_manager.set_production_cut("world", "gamma", 0.1 * u.mm)
sim.physics_manager.special_physics_constructors.G4OpticalPhysics = ENABLE_OPTICAL
...
# Sources
# -------------------------
# PET-like 511 keV source:
# - uniformly active sphere, 3 cm diameter
# - centered 25 cm in front of the module
# - isotropic emission inside a cone matched to the module size
source_radius = 15 * u.mm
source_distance = 25 * u.cm
source_center = [0, 0, -source_distance]
cone_margin = 1.00
theta_max = cone_margin * math.atan((block_xy / 2.0 + source_radius) / source_distance)

print("theta_max " + str(math.degrees(theta_max)) )

# Visible sphere to show the active source region in the Geant4 viewer.
source_vis = sim.add_volume("Sphere", "source_vis")
source_vis.mother = "world"
source_vis.rmin = 0
...
source_vis.material = "G4_AIR"
source_vis.translation = source_center
...
# Uniform active sphere
src.position.type = "sphere"
src.position.radius = source_radius
src.position.translation = source_center

# Isotropic cone broad enough to illuminate the full module from anywhere in the source sphere.
src.direction.type = "iso"
src.direction.momentum = [0, 0, 0]
src.direction.theta = [(180 - math.degrees(theta_max)) * u.deg, 180 * u.deg]
src.direction.phi = [0, 360 * u.deg]
src.n = 10000
